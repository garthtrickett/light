...
use self::{action::ActionHook, chats::Direction, configuration::Configuration};
...
impl State {
    ...
    pub fn mutate(&mut self, action: Action) {
        ...
        match action {
            ...
            Action::SetPopout(_) | Action::ClearPopout(_) | Action::SetDebugLogger(_) => todo!(),
        }
    ...
    pub fn load() -> Self {
        ...
        let mut content_object: State = serde_json::from_str(&contents).unwrap_or_default();
        content_object.logged_in = false;
        content_object
    }
